...
function Navbar() {
  ...
  return (
    <nav className='relative w-full flex justify-center items-center bg-amber-800 p-6 '>
         <div className='relative  w-full  md:space-y-2 px-1 py-5'>
            <div className='flex items-center space-y-3 md:space-y-0   justify-between  w-full '>
                  ...
                  <div className='md:hidden w-fit'>
                     ...
                     <div className='flex w-1/2 bg-white  justify-between  p-4  absolute left-0 z-10'>
                        <div className=''> {navLinks?.map((link) => 
                           <div key={link.title}>
                              <NavLink
                                 to={link.url}
                                 className={({ isActive, isPending }) => isActive ? "text-black underline" : "text-gray-400"}
                              >{link.title}
                              </NavLink>
                           </div>)}
                        </div>
               ...
               <div className='flex items-center space-x-4 '>
                  ...
                  {/* hide this SearchBar only on mobile screens */}
                  <Link to='your-cart'  className='relative'>
                     <svg className='inline-block' xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#000" d="M17 18c-1.11 0-2 .89-2 2a2 2 0 0 0 2 2a2 2 0 0 0 2-2a2 2 0 0 0-2-2M1 2v2h2l3.6 7.59l-1.36 2.45c-.15.28-.24.61-.24.96a2 2 0 0 0 2 2h12v-2H7.42a.25.25 0 0 1-.25-.25q0-.075.03-.12L8.1 13h7.45c.75 0 1.41-.42 1.75-1.03l3.58-6.47c.07-.16.12-.33.12-.5a1 1 0 0 0-1-1H5.21l-.94-2M7 18c-1.11 0-2 .89-2 2a2 2 0 0 0 2 2a2 2 0 0 0 2-2a2 2 0 0 0-2-2" /></svg>
                     <span className={`absolute right-0 -top-2 text-gray-400`}>{cartTotal}</span>
                  </Link>
            ...
            <div className='hidden md:flex md:gap-2 whitespace-nowrap md:items-center md:justify-center  '>
               {navLinks?.map((link) => (
               <div key={link.title}>
                  <NavLink
                     to={link.url}
            ...
            <SearchBar text={'Search for products...'} className=''/>
         </div>
      {/* </div> */}
    </nav>
  )
